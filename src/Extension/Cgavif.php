...
namespace Conseilgouz\Plugin\System\Cgavif\Extension;
...
final class Cgavif extends CMSPlugin implements SubscriberInterface
{
    protected $_avifs;
    protected $debugData;
    ...
    public function onAfterRender()
    {
        ...
        $gdInfo = gd_info();
        if(!isset($gdInfo['AVIF Support']) or !$gdInfo['AVIF Support']) {
            return;
        }
        ...
        if (!CgavifHelper::browserSupportAVIF()) {
            return;
        }
                ...
                if (is_string($filter)) {
                    ...
                    foreach ($filter as $onefilter) {
                        if ($onefilter->directory) {
                            $sHtml = $this->goavif($sHtml, $onefilter);
                        }
                    }
                } else {
                    if ($filter->directory) {
                        $sHtml = $this->goavif($sHtml, $filter);
                    }
                }
            }
        }
    ...
    private function goavif($sHtml, $onefilter)
    {
        ...
        if(count($extensions)) {
            ...
            $sHtml = preg_replace_callback(
                ...
                function ($match) use ($quality, $stored_time, $excludedArr, &$debugTarget, $regexPath) {
                    $img = $match[0];
                    $newImg = $this->imgToAVIF($img, $quality, $excludedArr, $stored_time, $regexPath, $match, $debugTarget);
                    $new = 'false';
    ...
    private function imgToAVIF($image, $quality = 100, $excluded = array(), $stored_time = 5, $regexPath = '', $fullRegex = '', &$debugTarget = [])
    {
        ...
        if(is_file($imgPath)) {
            if (!isset($this->_avifs[$imgHash])) {
                ...
                } else { // in media/plg_system_avif/_cache directory
                    $newImagePath = JPATH_ROOT .'/media/plg_system_cgavif/_cache/'.pathinfo($image)['dirname'].'/';
                }
                $newImage = $newImagePath . $imgInfo['filename'] . '.avif';

                // Delete avif image if is older than image
                if (is_file($newImage)) {
                ...
                if (!is_file($newImage)) {
                    if (is_file($imgPath)) {
                        try {
                            ...
                            imageavif($img, $newImage, $quality);
                            $bNew = 'true';
            ...
            $debugTarget[] = array(
                 'source' => $image,
                 'target' => $this->_avifs[$imgHash],
                 'new'    => $bNew
            );
            return $this->_avifs[$imgHash];
        }
    }
    public function onAjaxCgavif()
    {
        ...
        if (is_dir(JPATH_ROOT . '/media/plg_system_cgavif/_cache')) {
            Folder::delete(JPATH_ROOT . '/media/plg_system_cgavif/_cache');
        }
        echo 'ok';
    }
}
